...
## Phase 23 — TSP Generation Playback, Stats Panel, Walk-Tree List

The last major piece of item 6: TSP generation playback, the "other DNA facets" (genre mix, year spread, distance from seed, TSP improvement, annealing convergence as real readouts), and — instead of artist pictures, deliberately deferred — a live DOM "tree list" of TSP walk population turnover.

**Designing the TSP visual, deliberately avoiding a misleading metaphor**: the obvious idea — shrink an on-screen path connecting the blobs as tsp_score improves — would have been wrong. Blob screen positions are physics-simulated drift coordinates from `playback.ts`; they have no relationship to the actual genre+year distance TSP optimizes. A shrinking on-screen path would look meaningful while representing nothing real. Instead: each generation's *actual* best walk (`members[0]` — already sorted ascending by score, guaranteed by the backend's `_sort_walks` before building each `TSPGenerationEvent`, see `tsp.py`) drives a "chase" — a highlight visiting each track in that walk's real order, one at a time — while the real `tsp_score`/improvement number is reported as an actual figure in the stats panel, not inferred from geometry.

**Kept the highlight concept deliberately separate from `selected`** (`BlobState.highlighted`, `applyHighlight`), rather than reusing the existing selection flag. Reusing it would have meant one field driving two unrelated concerns (which tracks are chosen vs. where the chase pointer currently is) — precisely the class of scoping mistake Phase 22 had just caught (a physics parameter bleeding onto blobs it wasn't meant to affect). Entering the TSP phase also prunes the blob set down to exactly the final selected tracks (`pruneTo`, new pure function) — unrelated candidates that were never selected have nothing to do with TSP and must not keep sitting around being affected by TSP-phase state.

**`distance.ts`** mirrors `src/algorithms/distance.py`'s Jaccard + year distance formula and default weights exactly — verified with tests mirroring `tests/test_distance.py`'s cases input-for-input, output-for-output, proving numeric parity, not just "a similar-looking formula." Documented honestly: actual per-request weight overrides aren't part of the wire protocol (same reasoning `DistanceWeights` was excluded from codegen), so this is an approximation using the backend's *default* weights, not a byte-identical reproduction if a run ever used non-default ones.

**`stats.ts`** computes track count, genre mix, year min/max, and average distance from seed for whatever track set is currently emphasized (SA's reconstructed selection, a TSP generation's best walk, or the final result) — excludes the seed itself from the distance average (distance-to-self is always 0, would silently deflate it).

**`walkLineage.ts`**'s `diffGenerations` computes, generation to generation, which walk_ids newly appeared, which disappeared, and which are alive — pure, and directly wired to a DOM list in `index.html`/`main.ts` that appends new `<li>`s and fades out ones that stop surviving tournament selection. Verified against the real fixture that this isn't an empty gesture: 470 "appeared" events, 457 "disappeared" events, 409 distinct walk_ids ever alive across 51 generations of a population that stays exactly 20 wide — real, substantial turnover.

**Verified thoroughly, multiple layers**:
- Headless, pure-function replay (`web/scripts/verify_tsp_playback.ts`) against the real fixture: first generation's best score 6.80 -> last generation's 6.59, matching `vm.final.tspScore` (6.59) and `improvementPct` (3.1%) exactly. Confirmed the improvement is *not* monotonic generation-to-generation (a real dip at one point) — honest, because this GA has no elitism (tournament selection can lose the current best), not a bug to paper over.
- Live browser (CDP), first pass: confirmed all three phases, stats panel values, and walk-tree grow/shrink — but the "highlight color visible on canvas" check came back false. Caught immediately that this was a bug in the *verification script*, not the app: the check ran once, after the polling loop had already exited into "final" phase, where the highlight is correctly cleared. Re-ran it polling specifically during the TSP phase and confirmed the highlight genuinely appears on canvas.

62 web tests total, all Python tests still passing, `tsc` clean.

**Ongoing / Next:** artist pictures in blobs remain deliberately deferred, not forgotten. Item 6 is otherwise complete against its original description.
