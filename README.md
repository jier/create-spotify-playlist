...
What it animates: every candidate drifts in as a blob (color hashed from its genre set), then — for `strategy=sa` runs — steps through the actual simulated annealing trace, pulling selected tracks toward the seed as they're chosen (real positional convergence, not just a size change) while jitter cools with the anneal's temperature. Then a TSP phase prunes to the fixed final set and "chases" a highlight through each generation's actual best track ordering. A sidebar shows live stats (track count, year spread, average distance from seed, genre mix, SA energy, TSP score/improvement) and a "TSP walk population" list that grows and fades as track orderings appear and stop surviving selection. Not built yet: artist pictures in the blobs (deliberately deferred).
